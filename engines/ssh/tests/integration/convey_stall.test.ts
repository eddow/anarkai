...
describe('Convey Stall Reproduction', () => {
	...
	it('NPC should wake up when goods arrive at gate', {
		timeout: 15000,
	}, async () => {
		...
		for (let i = 0; i < 300; i++) {
			...
			if (i % 20 === 0) {
				...
				console.log(
					`Tick ${i}, storage executor: ${executorName}, movingGoods:`,
					Array.from(hive.movingGoods.keys()).map((k: any) => axial.key(k))
				)
			}
		}
		...
		for (let i = 0; i < 100; i++) {
			...
			if (i % 20 === 0) {
				console.log(
					`Tick ${i} after move started, sawmill incoming: ${sawmill.incomingGoods}, sawmill aGoodMovement: ${!!sawmill.aGoodMovement}`
				)
			}
		}
		...
		for (let i = 0; i < 200; i++) {
			...
			if (
				description === 'wait.incoming-goods' ||
				executorName === 'MultiMoveStep' ||
				description === 'convey'
			) {
				sawmillActive = true
				break
