...
describe('Single worker gather->sawmill regression', () => {
	it(
		'moves and transforms two starting wood with one worker without stalling the second reserved wood',
		{ timeout: 30000 },
		async () => {
	...
	it(
		'does not stall after dropping two loose woods into gatherer with one worker',
		{ timeout: 30000 },
		async () => {
			const engine = new TestEngine({ terrainSeed: 1234, characterCount: 0 })
			await engine.init()
	...
	it(
		'does not pin a single worker on incoming-only convey when a second storage also demands goods',
		{ timeout: 35000 },
		async () => {
			const engine = new TestEngine({ terrainSeed: 1234, characterCount: 0 })
			await engine.init()
			...
			try {
				...
				if (!gather || !sawmill || !storage) throw new Error('Expected gather/sawmill/storage to exist')
