...
describe('Mixed pipeline stall regression', () => {
	it(
		'keeps some convey work visible while wood-in and plank-out logistics overlap',
		{ timeout: 35000 },
		async () => {
			...
			try {
				...
				for (let i = 0; i < 320; i++) {
					...
					const mixedPipelineStall =
						gatherReservedWood > 0 && plankInTransit && !anyVisibleConveyJob
