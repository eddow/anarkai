...
setTraceDiagnosticReporter(({ channel, level, text }) => {
	const mappedLevel: TestDiagnosticEntry['level'] =
		level === 'assert failure'
			? 'assert'
			: level === 'log' || level === 'debug' || level === 'info' || level === 'trace'
				? 'error'
				: level
	recordDiagnostic(mappedLevel, [`[trace:${channel}:${level}] ${text}`])
})
...
reactiveOptions.onMemoizationDiscrepancy = (cached, fresh, fn: any, args, cause) => {
	...
	try {
		...
		const owner = args?.[0]
		if (owner) {
			console.error('Owner:', owner?.constructor?.name ?? typeof owner, brief(owner))
		}
