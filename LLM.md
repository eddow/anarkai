...
- **Build/zone/road actions** are direct game actions exposed through the browser toolbar. Do not reintroduce district-scoped build, zone, road, or save APIs.
...
- `Game.freightLines` is a **reactive `Set`** restored 1:1 from save patches — **no implicit gather lines are synthesized** (all lines are explicit).
- **One bay, many lines**: several lines may reference the same `freight_bay`. Prefer `findGatherFreightLines` / `findDistributeFreightLines`. **Runtime loops** should use `gatherSegmentAllowsGoodTypeForSegment` / `distributeSegmentAllowsGoodTypeForSegment` with the active segment; `freightLineAllowsGoodType` is UI/summary-only. `road-fret` is a freight-bay dock marker only; cargo lives in real storage or the docked `VehicleFreightDock`.
- **Explicit line lifecycle**: `Game.removeFreightLine(line)` (object ref). `createExplicitFreightLineDraftForFreightBay` + bay inspector add/remove are presets that create explicit lines.
- **Gather radius** lives on the **zone** step of a gather segment (`DEFAULT_GATHER_FREIGHT_RADIUS` for gather presets); distribute delivery cap is an optional **zone on the unload** step of a distribute segment.
...
- `freight_bay` is the named gather/distribute stop building and reuses the old `buildings.load` icon. The legacy `gather` alveolus content key is intentionally unsupported; update old saves/fixtures to `freight_bay`.
